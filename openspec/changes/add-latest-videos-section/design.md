...
The current landing page is a Vue 3 route at `src/pages/Home/HomePage.vue` with scoped CSS in `src/pages/Home/HomePage.css`, shared tokens in `src/styles/tokens.css`, and shared link styling through `src/components/BaseButton/BaseButton.vue`. The page already has the hero copy, primary YouTube channel CTA, and dark/accent visual language needed for the provided Figma direction.
...
- Add a focused latest-videos feature module under `src/features/latest-videos/`.
  ...
  - Alternative considered: Put all fetch and rendering logic directly in `HomePage.vue`. That would be faster initially but harder to test and reuse.
...
- Implement responsive styling in `HomePage.css` or a scoped feature CSS block using existing tokens.
